...
f5 = open('常德租房数据.csv', mode='a+', encoding='utf-8', newline='')
...
# eg:https://bj.lianjia.com/zufang/pg1rt200600000001f100500000001l3ra0rp2/
...
for i in range(0, 5):  # 城市遍历
    for manner in range(0, 2):  # 方式遍历
        for roomType in range(0, 4):  # 户型遍历
            for areaType in range(0, 6):  # 面积区间遍历
                for rentPrice in range(0, 12):  # 租金区间遍历
                    for page in range(1, 101):
                        if page % 10 == 0:  # 每 10 页休眠 1 秒
                            time.sleep(1)
                        url = 'https://' + city[i] + '.lianjia.com/zufang/' + 'pg' + str(page) + manners[manner] + roomTypes[roomType] + areaTypes[areaType] + rentPrices[rentPrice] + '/'
                        ...
                        for li in lis:
                            ...
                            dit['名称'] = title
                            if len(
                                    li.css('.content__list--item--des a::text').getall()) > 2:
                                ...
                                dit['具体地址'] = location3
                            if len(
                                    li.css('.content__list--item--des::text').
                                    getall()) == 8:
                            ...
                            if (len(lis) < 30 and len(lis) - cnt <= 8):  # 如果本页总租房数小于 30 且当前只剩八套，则该八套一定是推荐房源 ，进行break
                                break
